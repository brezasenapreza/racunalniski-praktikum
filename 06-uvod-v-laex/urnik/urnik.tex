...
kršitev omejitve $n$, z $n_n$ pa njeno utež. Z naslednjo formulo izračunamo
kakovost urnika:
$$k_1 \cdot u_1 + k_2 \cdot u_2 + \ldots + {k_n-1} \cdot u_{n-1} + k_n \cdot u$$
...
\subsection{Optimizacijske metode }azdelek
...
spremembo (npr. zamenjava $text{srecanje}_i$ in $text{srecanje}_j$) sedaj shranimo v
...
tem poskrbimo, da se lahko v isti lokalni maksimum vrnemo šele čez $s_{1}$ korakov, saj
...
\subsection{Končni algoritem }azdelek
...
\end{verbatim}
   ...
   se zaključi, če po $r$ korakih še vedno tičimo v istem lokalnem maksimumu.
